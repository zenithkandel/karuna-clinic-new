...
<section class="panel" style="margin-bottom:16px;">
    ...
    <form method="post">
        ...
        <?php if ($editUser): ?><input type="hidden" name="id"
                value="<?php echo (int) $editUser['id']; ?>"><?php endif; ?>

        <div class="grid-cards" style="grid-template-columns: repeat(auto-fit,minmax(240px,1fr));">
            <div class="form-row"><label>Username</label><input name="username" required
                    value="<?php echo htmlspecialchars($editUser['username'] ?? ''); ?>"></div>
            <div class="form-row"><label>Email</label><input name="email" type="email" required
                    value="<?php echo htmlspecialchars($editUser['email'] ?? ''); ?>"></div>
            <div class="form-row"><label>Full Name</label><input name="full_name"
                    value="<?php echo htmlspecialchars($editUser['full_name'] ?? ''); ?>"></div>
            <div class="form-row"><label>Role</label><select name="role"><?php foreach (['admin', 'staff'] as $role): ?>
                        <option value="<?php echo $role; ?>" <?php echo ($editUser['role'] ?? 'staff') === $role ? 'selected' : ''; ?>><?php echo ucfirst($role); ?></option><?php endforeach; ?>
                </select></div>
            <div class="form-row"><label>Password <?php echo $editUser ? '(leave empty to keep)' : ''; ?></label><input
                    name="password" type="password" <?php echo $editUser ? '' : 'required'; ?>></div>
            <div class="form-row" style="display:flex; align-items:center; gap:10px;"><input type="checkbox"
                    name="is_active" <?php echo !isset($editUser['is_active']) || (int) ($editUser['is_active'] ?? 1) === 1 ? 'checked' : ''; ?>><label>Active</label></div>
        </div>
        <div class="btn-row"><button class="btn btn-accent" type="submit"><i class="fas fa-floppy-disk"></i>
                <?php echo $editUser ? 'Update' : 'Create'; ?></button><?php if ($editUser): ?><a class="btn btn-ghost"
                    href="admin-users.php"><i class="fas fa-xmark"></i> Cancel</a><?php endif; ?></div>
    </form>
</section>
        ...
        <tbody>
            <?php foreach ($users as $user): ?>
                <tr>
                    ...
                    <td>
                        <div class="btn-row">
                            <a class="btn btn-ghost" href="admin-users.php?edit=<?php echo (int) $user['id']; ?>"><i
