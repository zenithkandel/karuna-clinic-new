...
<section class="panel" style="margin-bottom:16px;">
    ...
    <form method="post">
        ...
        <?php if ($editAppointment): ?><input type="hidden" name="id"
                value="<?php echo (int) $editAppointment['id']; ?>"><?php endif; ?>

        <div class="grid-cards" style="grid-template-columns: repeat(auto-fit,minmax(240px,1fr));">
            <div class="form-row"><label>Patient Name</label><input name="patient_name" required
                    value="<?php echo htmlspecialchars($editAppointment['patient_name'] ?? ''); ?>"></div>
            <div class="form-row"><label>Phone</label><input name="patient_phone" required
                    value="<?php echo htmlspecialchars($editAppointment['patient_phone'] ?? ''); ?>"></div>
            <div class="form-row"><label>Email</label><input name="patient_email" type="email"
                    value="<?php echo htmlspecialchars($editAppointment['patient_email'] ?? ''); ?>"></div>
            <div class="form-row"><label>Date</label><input name="appointment_date" type="date"
                    value="<?php echo htmlspecialchars($editAppointment['appointment_date'] ?? ''); ?>"></div>
            <div class="form-row"><label>Time</label><input name="appointment_time" type="time"
                    value="<?php echo htmlspecialchars($editAppointment['appointment_time'] ?? ''); ?>"></div>
            <div class="form-row"><label>Doctor</label><select name="doctor_id">
                    <option value="">Any</option><?php foreach ($doctors as $doctor): ?>
                        <option value="<?php echo (int) $doctor['id']; ?>" <?php echo isset($editAppointment['doctor_id']) && (int) $editAppointment['doctor_id'] === (int) $doctor['id'] ? 'selected' : ''; ?>>
                            <?php echo htmlspecialchars($doctor['name']); ?></option><?php endforeach; ?>
                </select></div>
            <div class="form-row"><label>Service Type</label><input name="service_type"
                    value="<?php echo htmlspecialchars($editAppointment['service_type'] ?? ''); ?>"></div>
            <div class="form-row"><label>Status</label><select
                    name="status"><?php foreach (['pending', 'confirmed', 'cancelled', 'completed'] as $st): ?>
                        <option value="<?php echo $st; ?>" <?php echo ($editAppointment['status'] ?? 'pending') === $st ? 'selected' : ''; ?>><?php echo ucfirst($st); ?></option><?php endforeach; ?>
                </select></div>
        </div>
        <div class="form-row"><label>Message</label><textarea name="message"
                rows="3"><?php echo htmlspecialchars($editAppointment['message'] ?? ''); ?></textarea></div>
        <div class="btn-row"><button class="btn btn-accent" type="submit"><i class="fas fa-floppy-disk"></i>
                <?php echo $editAppointment ? 'Update' : 'Create'; ?></button><?php if ($editAppointment): ?><a
                    class="btn btn-ghost" href="appointments.php"><i class="fas fa-xmark"></i> Cancel</a><?php endif; ?>
        </div>
    </form>
</section>
        ...
        <tbody>
            <?php foreach ($appointments as $appointment): ?>
                <tr>
                    ...
                    <td>
                        <div class="btn-row">
                            <a class="btn btn-ghost" href="appointments.php?edit=<?php echo (int) $appointment['id']; ?>"><i
