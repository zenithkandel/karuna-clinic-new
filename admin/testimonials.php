...
<section class="panel" style="margin-bottom:16px;">
    ...
    <form method="post">
        ...
        <?php if ($editItem): ?><input type="hidden" name="id"
                value="<?php echo (int) $editItem['id']; ?>"><?php endif; ?>
        ...
        <div class="grid-cards" style="grid-template-columns: repeat(auto-fit,minmax(260px,1fr));">
            <div class="form-row"><label>Patient Name</label><input name="patient_name" required
                    value="<?php echo htmlspecialchars($editItem['patient_name'] ?? ''); ?>"></div>
            <div class="form-row"><label>Rating (1-5)</label><input type="number" min="1" max="5" name="rating"
                    value="<?php echo htmlspecialchars((string) ($editItem['rating'] ?? 5)); ?>"></div>
            <div class="form-row"><label>Image</label><input name="image"
                    value="<?php echo htmlspecialchars($editItem['image'] ?? ''); ?>"></div>
        </div>
        <div class="form-row"><label>Testimonial</label><textarea name="testimonial"
                rows="4"><?php echo htmlspecialchars($editItem['testimonial'] ?? ''); ?></textarea></div>
        <div class="grid-cards" style="grid-template-columns: repeat(auto-fit,minmax(200px,1fr));">
            <div class="form-row" style="display:flex; align-items:center; gap:10px;"><input type="checkbox"
                    name="is_approved" <?php echo !empty($editItem) && (int) $editItem['is_approved'] === 1 ? 'checked' : ''; ?>><label>Approved</label></div>
            <div class="form-row" style="display:flex; align-items:center; gap:10px;"><input type="checkbox"
                    name="is_active" <?php echo !isset($editItem['is_active']) || (int) ($editItem['is_active'] ?? 1) === 1 ? 'checked' : ''; ?>><label>Active</label></div>
        </div>
        <div class="btn-row"><button class="btn btn-accent" type="submit"><i class="fas fa-floppy-disk"></i>
                <?php echo $editItem ? 'Update' : 'Create'; ?></button><?php if ($editItem): ?><a class="btn btn-ghost"
                    href="testimonials.php"><i class="fas fa-xmark"></i> Cancel</a><?php endif; ?></div>
    </form>
</section>
        ...
        <tbody>
            <?php foreach ($items as $item): ?>
                <tr>
                    ...
                    <td>
                        <div class="btn-row">
                            <a class="btn btn-ghost" href="testimonials.php?edit=<?php echo (int) $item['id']; ?>"><i
