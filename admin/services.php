...
<section class="panel" style="margin-bottom:16px;">
    ...
    <form method="post">
        ...
        <div class="form-row"><label>Name</label><input name="name" required value="<?php echo htmlspecialchars($editService['name'] ?? ''); ?>"></div>
        <div class="form-row"><label>Icon Class</label><input name="icon" value="<?php echo htmlspecialchars($editService['icon'] ?? 'fas fa-stethoscope'); ?>"></div>
        <div class="form-row"><label>Description</label><textarea name="description" rows="4"><?php echo htmlspecialchars($editService['description'] ?? ''); ?></textarea></div>
        <div class="form-row" style="display:flex; align-items:center; gap:10px;"><input type="checkbox" name="is_active" <?php echo !isset($editService['is_active']) || (int)$editService['is_active'] === 1 ? 'checked' : ''; ?>><label>Active</label></div>
        <div class="btn-row">
            <button class="btn btn-accent" type="submit"><i class="fas fa-floppy-disk"></i> <?php echo $editService ? 'Update' : 'Create'; ?></button>
            <?php if ($editService): ?><a class="btn btn-ghost" href="services.php"><i class="fas fa-xmark"></i> Cancel</a><?php endif; ?>
        </div>
    </form>
</section>
...
<section class="panel">
    ...
    <table class="data-table">
        <thead><tr><th>ID</th><th>Name</th><th>Icon</th><th>Description</th><th>Status</th><th>Actions</th></tr></thead>
        <tbody>
            <?php foreach ($services as $service): ?>
                <tr>
                    <td><?php echo (int)$service['id']; ?></td>
                    <td><?php echo htmlspecialchars($service['name']); ?></td>
                    <td><i class="<?php echo htmlspecialchars($service['icon']); ?>"></i> <?php echo htmlspecialchars($service['icon']); ?></td>
                    <td><?php echo htmlspecialchars($service['description'] ?? ''); ?></td>
                    <td><?php echo (int)$service['is_active'] === 1 ? 'Active' : 'Inactive'; ?></td>
                    <td>
                        <div class="btn-row">
                            <a class="btn btn-ghost" href="services.php?edit=<?php echo (int)$service['id']; ?>"><i class="fas fa-pen"></i> Edit</a>
